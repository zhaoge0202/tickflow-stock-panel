...
def test_confirm_cached_strategy_results_filters_failed_auction_gap(monkeypatch, tmp_path):
    monkeypatch.setattr(auction_confirmation, "_gate_status", lambda *_args: "open")
    ...
    cached = {
        ...
        "results": {
            "custom_dual_edge_focus": {
                ...
                "total": 2,
                "rows": [
                    {"symbol": "002059.SZ", "name": "云南旅游", "close": 6.07, "score": 80.0},
                    {"symbol": "002885.SZ", "name": "京泉华", "close": 10.00, "score": 90.0},
                ],
            },
        },
        "updated_at": 1,
    ...
    result = payload["results"]["custom_dual_edge_focus"]
    assert payload["gate_status"] == "confirmed"
    assert result["base_total"] == 2
    # 只有京泉华达标通过，云南旅游被剔除
    assert result["total"] == 1
    assert result["confirmed_total"] == 1
    assert [row["symbol"] for row in result["rows"]] == ["002885.SZ"]
    assert result["rejected_total"] == 1
    assert [row["symbol"] for row in result["rejected_rows"]] == ["002059.SZ"]
    assert "低于最低高开" in result["rejected_rows"][0]["auction_rejection_reason"]
