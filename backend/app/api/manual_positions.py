...
def list_positions(request: Request):
    return {"positions": manual_positions.load_all(_data_dir(request))}
...
def save_position(symbol: str, req: ManualPositionReq, request: Request):
    ...
    try:
        row = manual_positions.save_one(_data_dir(request), data)
...
def delete_position(symbol: str, request: Request):
    deleted = manual_positions.delete_one(_data_dir(request), symbol)
    if not deleted:
...
def import_positions(req: ImportReq, request: Request):
    try:
        rows = manual_positions.import_many(_data_dir(request), [p.model_dump() for p in req.positions])
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e)) from e
